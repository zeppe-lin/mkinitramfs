# mkinitramfs.cmdline.7.pod - documentation for mkinitramfs.cmdline(7)
...
B<mkinitramfs> provides a way to control configuration dynamically via
...
I<LABEL>, I<PARTUUID>.  B<Note>: I<PARTUUID> only supported in
B<util-linux>' L<blkid(8)>.
...
Pass C<--allow-discards> to L<cryptsetup(8)>.  Leave empty to disable.
...
L<mkinitramfs(8)>, L<mkinitramfs.config(5)>, L<mkinitramfs.hooks(7)>
