...
Specify directory where kernel modules stored.
...
B<mkinitramfs> uses the TMPDIR environment variable.  If set, it uses
subdirectories in the given directory to create its temporary working
directories.  Else it uses C</tmp> as default value for that purpose.
